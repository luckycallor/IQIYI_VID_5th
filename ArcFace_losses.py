...
def arcface_loss(embedding, labels, out_num, w_init=None, s=64., m=0.5):
    ...
    with tf.variable_scope('arcface_loss'):
        ...
        ori_norm_embd = tf.norm(embedding, axis=1, keep_dims=True)
        embedding_norm = tf.maximum(ori_norm_embd, 1e-6)
        embedding = tf.div(embedding, embedding_norm, name='norm_embedding')
        weights = tf.get_variable(name='embedding_weights', shape=(embedding.get_shape().as_list()[-1], out_num),
                                  initializer=w_init, dtype=tf.float32)
        ori_norm_w = tf.norm(weights, axis=0, keep_dims=True)
        weights_norm = tf.maximum(ori_norm_w, 1e-6)
    ...
    return output, ori_norm_embd, ori_norm_w
